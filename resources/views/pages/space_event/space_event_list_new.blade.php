                                ...
                                @if($spnew_event->isNotEmpty())
                                    @foreach ($spnew_event as $key=> $annual_event)
                                        <tr valign="middle">
                                            ...
                                            <td>
                                                <a href="{{ route('space_event.edit',$sp_event->event_id) }}"class="btn btn-outline-success btn-sm">
                                                <i class="bx bx-task" style="font-size: 20px"></i>
                                                </a>
                                                <a href="{{ route('space_event.edit',$sp_event->event_id) }}"class="btn btn-primary btn-sm">
                                                <i class='bx bx-edit' style="font-size: 20px"></i>
                                                </a>
                                                <a href="#" onclick="deleteEvent({{ $sp_event->event_id }})"class="btn btn-danger btn-sm">
                                                <i class='bx bxs-trash' style="font-size: 20px"></i>
                                                </a>
                                                <form id="event-delete-action-{{ $sp_event->event_id }}"action="{{ route('space_event.delete',$sp_event->event_id) }}"method="post">
                                                @csrf
                                                @method('delete')
                                                </form>
                                            </td>
